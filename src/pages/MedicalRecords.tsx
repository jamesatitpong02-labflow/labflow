...
export default function MedicalRecords() {
  ...
  // Function to fetch visits for a specific patient
  const fetchVisitsForPatient = async (patientId: string): Promise<VisitDetail[]> => {
    try {
      console.log('🔄 Fetching visits for patientId:', patientId);
      const visits = await apiService.getVisits();
      console.log('📊 Total visits from API:', visits.length);
      console.log('🔍 All visits:', visits);
      
      // Check what patientIds exist in visits
      const uniquePatientIds = [...new Set(visits.map(v => v.patientId))];
      console.log('🔍 Unique patient IDs in visits:', uniquePatientIds);
      
      // Filter visits by patientId (try multiple matching strategies)
      const matchingVisits = visits.filter(visit => {
        const visitPatientId = visit.patientId;
        console.log('🔍 Comparing:', { searchPatientId: patientId, visitPatientId });
        ...
        // Try matching with ObjectId conversion
        if (visitPatientId && patientId && visitPatientId.toString() === patientId.toString()) {
      ...
      console.log('🔍 Matching visits found:', matchingVisits.length);
      
      const patientVisits = await Promise.all(
        matchingVisits.map(async (visit) => {
            // Try to fetch lab orders and results for this visit
            let orders: OrderDetail[] = [];
            let results: ResultDetail[] = [];
            
            try {
              // Fetch lab orders for this visit
              const labOrders = await apiService.getOrders();
              orders = labOrders
                .filter(order => order.visitId === visit._id?.toString())
                .flatMap(order => 
                  order.labOrders.map(labOrder => ({
              ...
              console.log('⚠️ Could not fetch lab orders for visit:', visit.visitNumber);
            }
            
            try {
              // Fetch lab results for this visit
              const labResults = await apiService.getResults();
              results = orders.length > 0 ? labResults
                .filter(result => orders.some(order => order.orderId === result.orderId))
                .flatMap(result => 
                  result.testResults.map(testResult => ({
                    resultId: result._id?.toString() || '',
                    testName: testResult.testName || '',
                    result: testResult.result || '',
                    normalRange: testResult.referenceRange || '',
                    status: testResult.status === 'completed' ? 'normal' : 'pending',
                    attachedFiles: result.attachedFiles.map(file => ({
                      fileName: file.fileName,
                      fileData: file.fileData || '',
                      fileType: file.fileType,
                      uploadDate: new Date().toISOString() // Default upload date
                    })) || []
                  }))
                ) : [];
            } catch (error) {
              console.log('⚠️ Could not fetch lab results for visit:', visit.visitNumber);
            }
      ...
      console.log('✅ Found visits with orders/results for patient:', patientVisits);
    ...
    try {
      ...
      const medicalRecords = await Promise.all(
        rawData.map(async (record: any) => {
          ...
          // If no visits found in record, try to fetch them separately
          if (transformedVisits.length === 0 && (record.id || record.patientId)) {
            const searchId = record.id || record.patientId;
            console.log('🔄 No visits in record, fetching separately for ID:', searchId);
            console.log('🔍 Trying both record.id and record.patientId:', { recordId: record.id, patientId: record.patientId });
            try {
              transformedVisits = await fetchVisitsForPatient(searchId);
            } catch (error) {
